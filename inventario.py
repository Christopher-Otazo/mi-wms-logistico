...
def registrar_producto_nuevo():
    print("\n--- REGISTRO DE NUEVO SKU ---")
    sku = input("Ingrese SKU  ").upper()
    nombre = input("Nombre del componente:  " )
    try:
        cantidad = int(input("Stock inicial:   "))
        punto_critico = int(input("Punto de reposicion (Stock minimo):  "))
    except ValueError:
        print("Error: La cantidad y el punto critico deben ser numeros.")
        return

    ubicacion = input("Ubicacion en Bodega:  ").upper()
    ...
    try:
        ...
        conn.commit()
        print(f"Excelente! {nombre} ha sido ingresado al maestro de hardware.")
